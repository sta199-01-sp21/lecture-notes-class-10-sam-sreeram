...
The probability of a randomly selected patient receiving the placebo and 
having a marked improvement is 7/84.
  
- What is the probability a randomly selected patient received the placebo
  and the treatment?
  
0% probability because this is a disjoint probability.
  
- What is the probability a randomly selected patient had some improvement or
  was on the treatment?
  
48/84 is the probability of a randomly selected patient having some improvement 
or being on the treatment.
...
```{r simulate_draws}
draw_results <- sample(marbles, size = 1000, replace = TRUE)
```
...
```{r identify_events}
roll_results <- two_draw_results %>% 
  mutate(event1 = die_1^2 == die_2, event2 = abs(die_1-die_2) == 4)
roll_results
```
